...
def hello():
    ...
    return render_template('index.html',result=result, page=p, num=num, dealnum=dealnum, counttype=counttype,countlable=countlable,countentity=countentity)

@app.route('/blog/')
def blog():
    conn = db()
    blogid = request.args.get('blogid')
    # text = conn.site.find_one({'id':blogid})
